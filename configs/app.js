...
import '../src/opinions/opinion.model.js';
...
const routes = (app) => {
  ...
  app.get(`${BASE_PATH}/health`, (req, res) => {
    res.status(200).json({
      ...
      service: 'Gestor de Opiniones - Auth Service',
    });
  });
...
export const initServer = async () => {
  ...
  try {
    ...
    app.listen(PORT, () => {
      console.log(`Gestor de Opiniones API Server running on port ${PORT}`);
      console.log(`Health check: http://localhost:${PORT}${BASE_PATH}/health`);
    });
  } catch (err) {
    console.error(`Error starting Gestor de Opiniones API Server: ${err.message}`);
    process.exit(1);
  }
};